...
impl MetricViewer {
    pub fn new(syntheses: HashMap<MetricKey, Box<dyn MetricSynthesis>>) -> Self {
        Self { syntheses }
    }
    ...
    pub async fn scatter_chart_html(
        ...
        for key in keys {
            ...
            let synthesis = self.syntheses.get(key.as_ref());
            let span = match (queue, synthesis) {
                (Some(queue), _) => TimeSeries::span(queue, time_range.clone()),
                (None, Some(synthesis)) => synthesis
                    .as_ref()
                    .span(metrics, RangeAny::from_range(time_range.clone())),
                (None, None) => continue,
            };
